...
- ~~No AI API/model~~ — **relaxed 2026-08-29, explicitly requested by the user.** A Gemini-backed AI pipeline (`backend/src/orchestrator/AIOrchestrator.ts` and friends) is deployed server-side behind `POST /conversation/message` — see `06_AI_ARCHITECTURE.md` and `BUILD_LOG.md`. The Gemini key lives only as a Cloudflare Worker secret, never in `app/`. The Android app's Ask Legal AI screen does **not** call this endpoint yet — it still shows the old fixed placeholder reply. There is also no real legal knowledge base behind it yet (`LegalKnowledgeService` is a mock returning nothing), so real requests mostly get the honest "insufficient verified information" fallback by design, not a bug.
