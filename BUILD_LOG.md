...
- The Android app has **real authentication** (Supabase Auth, email OTP — see Auth, below) with a **persisted session** (survives app restart/process death), and **Incidents** are backed by the Cloudflare Worker + Supabase (see Backend, below). A real, Gemini-backed AI pipeline is deployed server-side (see Backend → AI pipeline) but **not yet called by the Android app** — Ask Legal AI still shows a fixed placeholder reply. Notifications and profile preferences (theme, text size, voice) are still in-memory UI.
...
#### AI pipeline (2026-08-29 → )

`POST /conversation/message` runs a real, deployed Gemini-backed pipeline — see `06_AI_ARCHITECTURE.md` for the design and `backend/src/orchestrator/AIOrchestrator.ts` for the implementation: Input Normalizer → Situation Classifier → Context Extractor → Risk Engine → Legal Knowledge Service → Authority Service → Action Planner → Response Generator → Citation Validator → Safety Validator.

- ✅ Deployed and verified live with a real Gemini key (free tier) and a real Supabase session: correctly classified domain/scenario, extracted only stated facts, assessed risk deterministically (correctly escalated a UPI-fraud message to HIGH; correctly kept a "police asked me to come to the station" message at LOW rather than treating a voluntary request as detention), and returned the honest "I don't have enough verified information to answer this reliably" fallback with no invented legal claims, since there's no real legal corpus behind it yet.
- **Model names are configurable, not hard-coded** (`GEMINI_MODEL`/`GEMINI_EMBEDDING_MODEL` in `wrangler.toml` → `src/config/env.ts`) — needed in practice almost immediately: the models this was originally written against (`gemini-2.0-flash`, `text-embedding-004`) had already been retired by Google by the time of first live testing. Current defaults, verified live: `gemini-3.5-flash-lite` (responds in ~1s with no extended-"thinking" overhead — matters here since one conversation turn makes up to 4 sequential structured-output calls) and `gemini-embedding-001`. Re-verify these are still current before assuming they work; Google's model lineup here has moved fast.
- `LegalKnowledgeService` and `AuthorityService` are still mock/interface-only (see `src/legal/`, `src/authority/`) — they return nothing by default, on purpose, since there is no real legal corpus or pgvector retrieval yet. Real legal claims can only ever come from a real implementation of these, never the LLM's own memory — `CitationValidator`/`SafetyValidator` enforce this even if a future prompt change tried to bypass it.
- **Not done**: Android doesn't call this endpoint; no real legal corpus/pgvector retrieval; no `conversations`/`messages` persistence table (the endpoint is stateless per call, round-tripping `previous_context` from the client); no mid-session Gemini-key rotation or 401-triggered retry logic (mirrors the same gap in Auth's session refresh, above).
...
- ~~No AI API keys, no live model~~ — relaxed 2026-08-29. A real Gemini-backed pipeline is deployed server-side (see Backend → AI pipeline, above); the key is a Worker secret, never in `app/`. **Ask Legal AI still returns one fixed placeholder reply per message** — the Android app doesn't call the new endpoint yet.
...
### 2026-08-29 — AI architecture foundation: Gemini-backed conversation pipeline, deployed and verified live
User handed over a numbered project knowledge base (`00_PROJECT_MASTER.md` through `19_PROJECT_STATUS.md`) specifying a modular (not multi-agent) AI architecture for legal first-aid, and asked for it to be built as a backend pipeline, then provided a real Gemini API key (free tier) to verify it live.

- **New pipeline** in `backend/src/`: `normalization/InputNormalizer`, `classification/SituationClassifier`, `context/ContextExtractor`, `risk/RiskEngine` (deterministic, no LLM), `legal/LegalKnowledgeService`, `authority/AuthorityService`, `planning/ActionPlanner`, `response/ResponseGenerator`, `validation/{CitationValidator,SafetyValidator}`, wired together by `orchestrator/AIOrchestrator.ts` and exposed as `POST /conversation/message` (see backend/README.md's new "AI pipeline" section for the full endpoint/module rundown).
- **Provider abstraction**: `ai/AIProvider` + `ai/EmbeddingProvider` interfaces, `GeminiAIProvider`/`GeminiEmbeddingProvider` as the only implementations — nothing else in the pipeline depends on Gemini directly. The Gemini key is a Cloudflare Worker secret (`GEMINI_API_KEY`, `wrangler secret put`), sent only via the `x-goog-api-key` header, never a URL, never logged, never in `app/`.
- **Legal grounding is structural, not a prompt instruction**: `LegalKnowledgeService`/`AuthorityService` are mock implementations that return nothing by default (there is no real legal corpus yet — see 07_RAG.md/08_LEGAL_KNOWLEDGE.md, not part of this pass). `ActionPlanner`/`ResponseGenerator` skip calling the LLM entirely when there's no evidence, so a fabricated-citation request has no code path to succeed through, not just a prompt telling the model not to. `CitationValidator` then strips any claim without a matching verified source before it reaches the client; `SafetyValidator` scans the final response for fabricated certainty, evasion/confrontation/obstruction advice, credential requests, and prompt-injection leakage, with a safe fallback response if anything trips.
- **68 unit/integration tests added** (`backend/test/`, Vitest) covering all 10 required modules plus the full red-team scenario list (unsupported jurisdiction, ambiguous scenario, fabricated-section request, prompt injection, evade-arrest, confront-police, insufficient evidence) — all passing. Caught and fixed a real bug during testing: `SafetyValidator`'s confrontation-detection regex had no negation awareness, so it would have flagged the app's own safety advice ("Do not argue with the officer.") as if it were *encouraging* confrontation; fixed with a negation-lookback check before flagging a match.
- **Deployed and verified live** against the real Gemini API (not just mocked in tests) using the user's own free-tier key, through a real authenticated Supabase session: correctly classified domain/scenario for two different messages, extracted only user-stated facts, and correctly escalated risk to HIGH for an active UPI-fraud report while keeping a "police asked me to come to the station" message at LOW. Discovered live that the models this was written against (`gemini-2.0-flash`, `text-embedding-004`) had already been retired by Google — re-verified current model availability directly against the API and switched defaults to `gemini-3.5-flash-lite` (no extended-thinking overhead, ~1s responses — matters since one turn makes up to 4 sequential model calls) and `gemini-embedding-001`.
- **Conflicts found in the new knowledge base vs. this repo's actual state, reported rather than silently resolved**: `19_PROJECT_STATUS.md` says the project is still at "Planning / architecture preparation" with steps like "Create backend" listed as not-yet-started, which is stale — not corrected in this pass (out of scope; flagging for whoever maintains it). `10_API.md` specifies `/api/v1`-prefixed, backend-proxied auth endpoints; this repo's actual API is unprefixed with direct Android→Supabase auth (a prior, deliberate decision — see 2026-08-28's backend entry below). The new `/conversation/message` route follows the existing unprefixed convention rather than introducing a third style unilaterally.
- **Not done**: Android's Ask Legal AI screen doesn't call this endpoint yet (still the old fixed placeholder); no real legal corpus/pgvector retrieval; no conversation persistence table; no mid-session token-refresh/retry for the Gemini call chain.
